...
import { backgroundColor, APIUrl, newReceipt } from "../../constants";
...
export const TableConstructor: React.FC<props> = (props) => {
  ...
  const [dataJobOffer, setDataJobOffer] = useState<any[]>([newReceipt]);
  ...
  const actualisationAllData =()=>{setLoagJobOffer(loagJobOffer+1);};
  ...
  useEffect(() => {
    setActivLoading(true)
    axiosGetWithPage("/receipts",page,pageNumber,myToken,setDataJobOffer,()=>{setActivLoading(false)},null)
  }, [loagJobOffer,page,pageNumber,myToken])
        ...
        <div className="dataTable-dropdown">
          <label>
            <select className="dataTable-selector">
              <option value="5" onClick={() => setPageNumber(5)}>
                5
              </option>
              <option value="10" onClick={() => setPageNumber(10)} selected>
                10
              </option>
              <option value="15" onClick={() => setPageNumber(15)}>
                15
              </option>
              <option value="20" onClick={() => setPageNumber(20)}>
                20
              </option>
              <option value="25" onClick={() => setPageNumber(25)}>
                25
              </option>
            </select>{" "}
            entité par pages
      ...
      <div className="dataTable-bottom p-2">
        <div className="dataTable-info">
          {"Affiche " +
            ...
            " à " +
            Math.min(page * pageNumber, items.length) +
            " sur " +
            items.length +
            " données"}
        </div>
        <nav className="dataTable-pagination">
          {HorizontalPagination(longPage, pageNumber, page, setPage, tri)}
        </nav>
      </div>
